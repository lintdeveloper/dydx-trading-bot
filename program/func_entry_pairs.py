...
from func_utils import format_number, convert_to_step_size
...
from dydx3.constants import SYNTHETIC_ASSET_MAP, ASSET_RESOLUTION
    ...
    if len(series_1) > 0 and len(series_1) == len(series_2):
      ...
      if abs(z_score) >= ZSCORE_THRESH:
        ...
        if not is_base_open and not is_quote_open:
          ...
          accept_base_price = Decimal(float(base_price)) * Decimal(1.01) if z_score < 0 else Decimal(float(base_price)) * Decimal(0.99)
          accept_quote_price = Decimal(float(quote_price)) * Decimal(1.01) if z_score > 0 else Decimal(float(quote_price)) * Decimal(0.99)
          failsafe_base_price = Decimal(float(base_price)) * Decimal(0.05) if z_score < 0 else Decimal(float(base_price)) * Decimal(1.7)
          ...
          # Get size
          base_quantity = Decimal(1) / Decimal(base_price) * Decimal(USD_PER_TRADE)
          quote_quantity = Decimal(1) / Decimal(quote_price) * Decimal(USD_PER_TRADE)
